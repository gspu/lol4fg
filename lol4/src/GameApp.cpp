...
Ogre::String GameApp::getSavegameSettings()
{
    ...
    mainElem->SetAttribute("playersgid",str(playerSGID));
...
void GameApp::loadGame(Ogre::String name)
{
    ...
    if(!maybePlayer || maybePlayer->getType() != otChar)
        throw Ogre::Exception(0,"Error while loading savegame "+name+": Object with SGID "+str(playerSGID)+" does not exist or is not a character!","GameApp::loadGame");
