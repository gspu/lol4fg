...
SimpleSound::SoundManager::DistanceModel StandardApplication::parseDistanceModel(Ogre::String str)
{
	if(str == "INVERSE_DISTANCE")
	{
		return SimpleSound::SoundManager::INVERSE_DISTANCE;
	}
	else if(str == "INVERSE_DISTANCE_CLAMPED")
	{
		return SimpleSound::SoundManager::INVERSE_DISTANCE_CLAMPED;
	}
	else if(str == "LINEAR_DISTANCE")
	{
		return SimpleSound::SoundManager::LINEAR_DISTANCE;
	}
	else if(str == "LINEAR_DISTANCE_CLAMPED")
	{
		return SimpleSound::SoundManager::LINEAR_DISTANCE_CLAMPED;
	}
	else if(str == "EXPONENT_DISTANCE")
	{
		return SimpleSound::SoundManager::EXPONENT_DISTANCE;
	}
	else if(str == "EXPONENT_DISTANCE_CLAMPED")
	{
...
void StandardApplication::spellListToXml(TiXmlElement *elem,SpellList list)
{
	for(SpellList::iterator itr=list.begin();itr!=list.end();itr++)
	{
		...
		if(cur->caster)
			child->SetAttribute( "caster",str(cur->caster->getSGID()).c_str() );
		if(cur->target)
			child->SetAttribute("target",str(cur->target->getSGID()).c_str());
		child->SetAttribute("lifetime",str(cur->lifetime).c_str());
...
void StandardApplication::inventoryToXml(TiXmlElement *elem, Inventory *inv)
{
	...
	for(itr = inv->itemVector.begin();itr != inv->itemVector.end();itr++)
	{
		...
		else
		{
			...
			XMLNode->SetAttribute("count",str(cur.count).c_str());
		}
...
void StandardApplication::statsToXml(TiXmlElement *elem, Stats st)
{
	...
	XMLNode->SetAttribute("value",str(st.mStats[st_strength]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "agility" );
	XMLNode->SetAttribute("value",str(st.mStats[st_agility]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "magic" );
	XMLNode->SetAttribute("value",str(st.mStats[st_magic]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "maxHP" );
	XMLNode->SetAttribute("value",str(st.mStats[st_maxHP]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "maxMP" );
	XMLNode->SetAttribute("value",str(st.mStats[st_maxMP]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "resistBlunt" );
	XMLNode->SetAttribute("value",str(st.mStats[st_resistBlunt]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "resistCut" );
	XMLNode->SetAttribute("value",str(st.mStats[st_resistCut]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "resistPierce" );
	XMLNode->SetAttribute("value",str(st.mStats[st_resistPierce]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "resistFire" );
	XMLNode->SetAttribute("value",str(st.mStats[st_resistFire]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "resistIce" );
	XMLNode->SetAttribute("value",str(st.mStats[st_resistIce]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "resistSpark" );
	XMLNode->SetAttribute("value",str(st.mStats[st_resistSpark]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "resistPoison" );
	XMLNode->SetAttribute("value",str(st.mStats[st_resistPoison]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "resistDark" );
	XMLNode->SetAttribute("value",str(st.mStats[st_resistDark]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "resistLight" );
	XMLNode->SetAttribute("value",str(st.mStats[st_resistLight]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "hpRegenRate" );
	XMLNode->SetAttribute("value",str(st.mStats[st_hpRegenRate]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "manaRegenRate" );
	XMLNode->SetAttribute("value",str(st.mStats[st_manaRegenRate]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "walkSpeed" );
	XMLNode->SetAttribute("value",str(st.mStats[st_walkSpeed]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "runSpeed" );
	XMLNode->SetAttribute("value",str(st.mStats[st_runSpeed]).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "ancientMagic" );
	XMLNode->SetAttribute("value",str(st.ancientMagic).c_str());
	elem->LinkEndChild(XMLNode);

	XMLNode = new TiXmlElement( "mana" );
	XMLNode->SetAttribute("value",str(st.mana).c_str());
	elem->LinkEndChild(XMLNode);
...
void StandardApplication::collisionDataToXml(TiXmlElement *elem,CollisionData data)
{
	...
	elem->SetAttribute("params",str(data.collistionParams).c_str());
	elem->SetAttribute("mass",str(data.mass).c_str());
	elem->SetAttribute("filename",data.collisionFile.c_str());
	elem->SetAttribute("orientationOffset",str(data.orientationOffset).c_str());
	elem->SetAttribute("positionOffset",str(data.positionOffset).c_str());
	elem->SetAttribute("upVector",str(data.upVector).c_str());
...
void StandardApplication::worldArtToXml(WorldArtData wData, TiXmlElement *nod)
{
	...
	case WT_BILLBOARD:
		...
		if(wData.bbColor != Ogre::ColourValue::White)
			nod->SetAttribute("billboard_color",str(wData.bbColor).c_str());
		break;
	...
	case WT_PLANE:
		type = "plane";
		nod->SetAttribute("normal",str(wData.pData.normal).c_str());
		nod->SetAttribute("constant",str(wData.pData.constant).c_str());
		nod->SetAttribute("width",str(wData.pData.width).c_str());
		nod->SetAttribute("height",str(wData.pData.height).c_str());
		nod->SetAttribute("xSeg",str(wData.pData.xSeg).c_str());
		nod->SetAttribute("ySeg",str(wData.pData.ySeg).c_str());
		nod->SetAttribute("numTexCoordSets",str(wData.pData.numTexCoordSets).c_str());
		nod->SetAttribute("uTile",str(wData.pData.uTile).c_str());
		nod->SetAttribute("vTile",str(wData.pData.vTile).c_str());
		nod->SetAttribute("upVector",str(wData.pData.up).c_str());
	...
	case WT_RIBBONTRAIL:
		if(type == "")
			type = "ribbontrail";
		nod->SetAttribute("dynamic",str(wData.bbcData.dynamic).c_str());
		nod->SetAttribute("maxElements",str(wData.bbcData.maxElements).c_str());
		nod->SetAttribute("numberOfChains",str(wData.bbcData.numberOfChains).c_str());
		nod->SetAttribute("useColours",str(wData.bbcData.useColours).c_str());
		nod->SetAttribute("useTextureCoords",str(wData.bbcData.useTextureCoords).c_str());
	...
	if(wData.positionOffset != Ogre::Vector3::ZERO)
		nod->SetAttribute("position_offset",str(wData.positionOffset).c_str());
	if(wData.orientationOffset != Ogre::Quaternion::IDENTITY)
		nod->SetAttribute("orientation_offset",str(wData.orientationOffset).c_str());
	nod->SetAttribute("scale",str(wData.scale));
	/*if(wData.scale != Ogre::Vector3::UNIT_SCALE)
		nod->SetAttribute("scale",str(wData.scale).c_str());*/
...
TiXmlElement *StandardApplication::chStatsToXml(ChStat stat)
{
	...
	res->SetAttribute("value",str(stat.value).c_str());
	switch(stat.type)
	{
    case st_strength:
...
void StandardApplication::explosionDataToXml(TiXmlElement *elem,ExplosionData data)
{
	...
	lsElem->SetAttribute("value",str(data.lifespan).c_str());
	elem->LinkEndChild(lsElem);

	TiXmlElement *slElem = new TiXmlElement("sound_loop");
	slElem ->SetAttribute("value",str(data.loopSound).c_str());
...
void StandardApplication::missileDataToXml(TiXmlElement *elem, MissileData data)
{
	...
	velElem->SetAttribute("value",str(data.velocity).c_str());
	cntElem->SetAttribute("value",str(data.count).c_str());
	delElem->SetAttribute("value",data.sound.c_str());
	angElem->SetAttribute("value",str(data.multimissile_delay).c_str());
	repElem->SetAttribute("value",str(data.multimissile_angle).c_str());
	sndElem->SetAttribute("value",data.sound.c_str());
...
collision_type StandardApplication::stringToColType(Ogre::String str)
{
	...
	if(str == "box")
		...
		return CT_CAPSULE;
	else if(str == "champfercylinder")
		return CT_CHAMFERCYLINDER;
	else if(str == "cone")
		return CT_CONE;
	else if(str == "convexhull")
...
SpellHitType StandardApplication::stringToSpellHit(Ogre::String str)
{
	Ogre::StringUtil::toLowerCase(str);
	if(str=="missile")
		return shMissile;
...
SpellEffectType StandardApplication::stringToSpellEffect(Ogre::String str)
{
	
	Ogre::StringUtil::toLowerCase(str);
	if(str=="damage")
		return seDamage;
	else if(str=="modstats")
		return seModStats;
	else if(str=="absorb")
		return seAbsorb;
	else if(str=="script")
...
{	
	...
	elem->SetAttribute("splashradius",str(data.splashRadius).c_str());
...
void StandardApplication::setDebugText(Ogre::String str,bool add)
{
...
UniKey StandardApplication::stringToUniKey(Ogre::String str)
{
	UniKey res;
