...
ExplosionObject::ExplosionObject(TiXmlElement *XMLNode,Level *lvl,Ogre::Vector3 positionOffset)
{
	...
	if(XMLNode->Attribute("target"))
	{
		...
		target = ObjectPtr(lvl,casterID );
	}

	init(ObjectPtr(caster),target,castitem,lvl,pos,_effectonly,_nolifetime,ornt,scale,SaveGameID);
	lifetime = _lifetime;
	if(mSnd)
	{
...
void ExplosionObject::init(ObjectPtr caster,ObjectPtr target,gamedata_damagedealer *castingItem,Level *lvl,Ogre::Vector3 pos,bool effect_only, bool liveForever,Ogre::Quaternion ornt,Ogre::Vector3 scale,int SaveGameID)
{
    ...
    if(!target.isNull() && target->getType() == gtCharacter)
	    mTarget = target;
    else
        mTarget.setNull();
	//jetzt der eigentliche effekt
    if(!target.isNull() && !effectOnly)
    {
		//nun schauen. spell?
		...
		else
		{
			...
			//hier auch damage
            target->onDamage(item->rangedDamage,mCaster.obj);
		}
	    if(spell)
            target->AddSpell(spell,mCaster.obj);
...
//	Ogre::String id = obj->getID(); = ID
    ...
    if(!mTarget.isNull())
		xmlElem->SetAttribute("target",str(mTarget->getSGID()));
...
/*
	...
	xmlElem->SetAttribute("lifetime",str(lifetime).c_str());
