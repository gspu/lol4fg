...
namespace QtOgre
{
	...
	public:
		...
		//gets the value how much of a certain layer is visible when it has layers above it
		//blendValue of the point * this coverage value = should be the final value
		Ogre::Real getCoverageAboveLayer(Ogre::Terrain *terrain, Ogre::uint8 layer, size_t x, size_t y);

		void setCoverageAboveLayer(Ogre::Terrain *terrain, Ogre::uint8 layer, size_t x, size_t y, Ogre::Real val);
		...
		//void paintOntoTerrain(Ogre::Terrain *terrain, int  const Ogre::Vector3& centrepos, Ogre::Real timeElapsed);
    ...
    //				mDebugText("jetzt");
		...
		//this is called as soon as the new level is constructed, but the old one still exists
		void levelChangeBegin(Level *newLevel);
