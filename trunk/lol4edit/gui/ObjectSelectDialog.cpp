...
void ObjectSelectDialog::selectEntry(QTreeWidgetItem *item, int column)
{
	if(item->childCount())
		return;
	GamedataTreeItem *objectItem = dynamic_cast<GamedataTreeItem*>(item);
	if(!objectItem)
		return;
...
void ObjectSelectDialog::fillLists()
{
	...
	new GamedataTreeItem(elseItem,"Entrance",NULL,eitEntrance);
	new GamedataTreeItem(elseItem,"Light",NULL,eitLight);
	new GamedataTreeItem(elseItem,"Plane",NULL,eitPlane);
	new GamedataTreeItem(elseItem,"Billboard",NULL,eitBillboard);
	new GamedataTreeItem(elseItem,"Waterplane",NULL,eitWaterPlane);	
		...
		itr != app->gameDataMap.end();itr++)
	{
		...
		{
		case gtDynamic: 
			new GamedataTreeItem(dynamicItem,str,data);
			break;
		case gtContainer: 
			new GamedataTreeItem(containerItem,str,data);
			break;
		case gtItem: 
			new GamedataTreeItem(itemItem,str,data);
			break;
		case gtSpell:
			new GamedataTreeItem(spellItem,str,data);
			break;
		case gtCharacter:
			new GamedataTreeItem(charItem,str,data);
			break;
		case gtDoor: 
			new GamedataTreeItem(doorItem,str,data);
			break;		
		}
	...
	while(psitr.hasMoreElements())
	{
		...
		new GamedataTreeItem(particleItem,name,NULL,eitStaticPS);		
	...
	{
		Ogre::String str = *i;
		new GamedataTreeItem(meshItem,str.c_str(),NULL,eitStaticMesh);
	}		
	//END list of meshes
	...
	{
		Ogre::String str = *i;
		new GamedataTreeItem(dsItem,str.c_str(),NULL,eitStaticDotScene);
	}		
