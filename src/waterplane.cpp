...
TiXmlElement* WaterPlane::getXmlElem(Ogre::Vector3 startPosition,bool savegame)
{
	...
	xmlElem->SetAttribute("density",str(mFluidData.density));
	xmlElem->SetAttribute("linearViscosity",str(mFluidData.linearViscosity));
	xmlElem->SetAttribute("angularViscosity",str(mFluidData.angularViscosity));
	
	//planeElem->SetAttribute("normal",str(wData.pData.normal).c_str());
	TiXmlElement* planeElem = new TiXmlElement("plane");
	//planeElem->SetAttribute("constant",str(mFluidData.plane.constant).c_str());
	planeElem->SetAttribute("width",str(mFluidData.plane.width).c_str());
	planeElem->SetAttribute("height",str(mFluidData.plane.height).c_str());
	planeElem->SetAttribute("xSeg",str(mFluidData.plane.xSeg).c_str());
	planeElem->SetAttribute("ySeg",str(mFluidData.plane.ySeg).c_str());
	planeElem->SetAttribute("numTexCoordSets",str(mFluidData.plane.numTexCoordSets).c_str());
	planeElem->SetAttribute("uTile",str(mFluidData.plane.uTile).c_str());
	planeElem->SetAttribute("vTile",str(mFluidData.plane.vTile).c_str());
	planeElem->SetAttribute("upVector",str(mFluidData.plane.up).c_str());
