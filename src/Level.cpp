...
{
	//mLog("setMusic("+str(battle)+": "+ptrToString(this));
	...
	case otStatic:
		...
		{	
			...
			case WT_PARTICLESYSTEM:
				...
				break;
			default:
				mLog("Error: could not create static object of type "+str(staticType));
...
void Level::update(Ogre::Real time)
{
	...
	*/
	...
	if(time != 0)
	{
		...
		//	//mLog("mWorld->update("+str(mWorldUpdate)+"), mWorldElapsed="+str(mWorldElapsed));
		//}
		//}
	}
...
void Level::placePlayer(Ogre::String entranceName,GameChar *oldPlayer)
{
	...
	{
		PosOrient entr = entrances[entranceName];
...
//        fallback = true;
//		texturesPerPass = 3;
//		textureFragmentName = "ET/Programs/FallbackSplat_";//+str(texturesPerPass)+"tex";
//	}
//	else
//	{
...
//		}
//		//normal
//		textureFragmentName = "ET/Programs/Splat_";//+str(texturesPerPass)+"tex";
//	}
//	
...
//		
//		pass->setFragmentProgram(textureFragmentName+str(texturesThisPass)+"tex");
...
//			tex->setTextureName(splattingBasename+str( passnr*covMapsPerPass+covmapnr ));
//		}
//		//jetzt die texturen
...
//    {
//        Ogre::String name = "terrain/coverage."+str(i)+".png";
...
void Level::processSGID(GameObject *obj)
{
	...
	if(curId)
	{
		if(saveableObjects.find(curId) != saveableObjects.end())
		{
			mLog("Warnung: SGID "+str(curId)+" wird bereits verwendet!");
...
Ogre::String Level::getSavegameXML()
{
	...
	levelElement->SetAttribute("version",str(CUR_LEVEL_VERSION).c_str());
	levelElement->SetAttribute("last_sgid",str(lastSGID).c_str());
		...
		itr != saveableObjects.end();itr++)
	{
		...
		if(!obj)
		{
			...
			xmlElem->SetAttribute("SGID",str(itr->first));
...
//	//			//mLog("hole_x="+str(hole_x)+", hole_z="+str(hole_z)+", x="+str(x)+", z="+str(z));
...
//	StandardApplication::getSingletonPtr()->setDebugText("save to "+filename);
	...
	*/
	...
	if(mFogMode != Ogre::FOG_NONE)
	{
		...
		xmlElem->SetAttribute("colour",str(mFogColor).c_str());
		xmlElem->SetAttribute("expDensity",str(mFogDensity).c_str());
		xmlElem->SetAttribute("linearStart",str(mFogStart).c_str());
		xmlElem->SetAttribute("linearEnd",str(mFogEnd).c_str());
		levelElement->LinkEndChild(xmlElem);
	}
	...
	xmlElem->SetAttribute("wait_min",str(wait_min).c_str());
	xmlElem->SetAttribute("wait_max",str(wait_max).c_str());
	...
	//"special"
	/*for(std::map<Ogre::String, PosOrient>::iterator itr = entrances.begin();itr != entrances.end();itr++)
	{
		Ogre::String name = itr->first;
		PosOrient psor = itr->second;
...
Ogre::String Level::getUniqueEntranceName(Ogre::String base)
{
	...
	int i = 0;
	while(entrances.find(base+str(i)) != entrances.end())
	{
		i++;
	}
	return base+str(i);
...
//
//	////mLog("tileLoaded: x="+str(page_x)+", Z="+str(page_z));
//	////Ogre::ResourceManager()::getsing
//	////Ogre::String filename = "plsm2_"+worldGeometryFile+"_"+str(page_x)+"_"+str(page_z)+"_"+str(tile_x)+"_"+str(tile_z)+".collision";
//	//
//	//
...
//	//unsigned int NumPages = opt->NumPages;
//
////	mLog("pageLoaded: x="+str(page_x)+", Z="+str(page_z));
//	//Ogre::ResourceManager()::getsing
//	//Ogre::String filename = "plsm2_"+worldGeometryFile+"_"+str(page_x)+"_"+str(page_z)+"_"+str(tile_x)+"_"+str(tile_z)+".collision";
//	Ogre::ResourceGroupManager *rgm = Ogre::ResourceGroupManager::getSingletonPtr();
//	TreeCollisionSerializer* serializer=new TreeCollisionSerializer();
//
//
//	Ogre::String filename = "plsm2_"+plsm2MapName+"_"+str(page_x)+"_"+str(page_z)+".collision";
//	if(rgm->resourceExists("Collision",filename))
//	{
//		//ok, col kann geladen werden
//		Ogre::DataStreamPtr ptr = rgm->openResource(filename,"Collision");
...
//	else
//	{
//		mLog("WARNUNG: für Page ("+str(page_x)+","+str(page_z)+") ist keine Collisionsdatei vorhanden");
//	}
//
