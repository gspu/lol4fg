...
WaterMesh::WaterMesh(const String& meshName, Vector3 planeSize, int complexity)
{
	...
	for(b=0;b<3;b++) {
		...
		for(y=0;y<=complexity;y++) {
			for(x=0;x<=complexity;x++) {
                ...
                vertex[0]=(float)(x) / (float)(complexity) * (float) planeSize.x ;
				vertex[1]= 0 ; // rand() % 30 ;
                vertex[2]=(float)(y) / (float)(complexity) * (float) planeSize.z ;
			}
		}
	}

	AxisAlignedBox meshBounds(0,0,0,
        planeSize.x,0, planeSize.z);
...
/* ========================================================================= */
void WaterMesh::prepareCircleBuffers()
{
    int i, lvl;
    // static buffer for position and normals
	circle_posnormVertexBuffer =
		HardwareBufferManager::getSingleton().createVertexBuffer(
			6*sizeof(float), // size of one vertex data
			4, // number of vertices
			HardwareBuffer::HBU_STATIC_WRITE_ONLY, // usage
			false); // no shadow buffer
	float *posnormBufData = (float*) circle_posnormVertexBuffer->
	...
	circle_posnormVertexBuffer->unlock();

	// static buffers for 16 sets of texture coordinates
	circle_texcoordsVertexBuffers = new HardwareVertexBufferSharedPtr[16];
	for(lvl=0;lvl<16;lvl++) {
		circle_texcoordsVertexBuffers[lvl] =
			HardwareBufferManager::getSingleton().createVertexBuffer(
				2*sizeof(float), // size of one vertex data
				numVertices, // number of vertices
				HardwareBuffer::HBU_STATIC_WRITE_ONLY, // usage
				false); // no shadow buffer
		float *texcoordsBufData = (float*) circle_texcoordsVertexBuffers[lvl]->
			lock(HardwareBuffer::HBL_DISCARD);
		...
		circle_texcoordsVertexBuffers[lvl]->unlock();
	}
	...
	circle_indexBuffer =
		HardwareBufferManager::getSingleton().createIndexBuffer(
			...
			6,
			HardwareBuffer::HBU_STATIC_WRITE_ONLY);
	circle_indexBuffer->writeData(0,
		circle_indexBuffer->getSizeInBytes(),
		faces,
		true); // true?
}
/* ========================================================================= */
...
//****************************************************************************+
void WaterMesh::clearCircleBuffers()
{
	circle_posnormVertexBuffer = HardwareVertexBufferSharedPtr() ;
	circle_indexBuffer = HardwareIndexBufferSharedPtr() ;
	for(int i=0;i<16;i++) {
		circle_texcoordsVertexBuffers[i] = HardwareVertexBufferSharedPtr() ;
	}
	delete [] circle_texcoordsVertexBuffers;
}
//********************************************************************************

void WaterMesh::prepareCircleMaterial()
{
...
//	int i,lvl ;
	...
	int numVertices = 4 ;
    WaterMesh *mParent = mParentObject->getWaterMesh();
	...
	VertexBufferBinding *vbind = subMesh->vertexData->vertexBufferBinding ;
	vbind->setBinding(0, mParent->circle_posnormVertexBuffer);
	vbind->setBinding(1, mParent->circle_texcoordsVertexBuffers[0]);
	...
	subMesh->indexData->indexBuffer = mParent->circle_indexBuffer;
	subMesh->indexData->indexStart = 0;
...
WaterCircle::WaterCircle(WaterPlane *parent, Real x, Real y)
{
    mParentObject = parent;
	name = "WaterCircle"+ptrAsStr();//name ;
	_prepareMesh();
    SceneManager *sceneMgr = mParentObject->getLevel()->getSceneManager();
...
void WaterCircle::updateTextureLevel()
{
	subMesh->vertexData->vertexBufferBinding->setBinding(1, mParentObject->getWaterMesh()->circle_texcoordsVertexBuffers[lvl]);
}