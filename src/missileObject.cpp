...
void MissileObject::init(GameObject *caster, gamedata_damagedealer *source, Level *lvl, Ogre::Vector3 pos, Ogre::Quaternion ornt, Ogre::Vector3 v0,int SaveGameID)
{
	if(!source->Missile)
	{
...
//	with->AddEffects(mData.effects,caster);
    ...
    if(with)
    {
		...
		else
		{
			...
			//hier auch damage
            with->onDamage(item->rangedDamage,mCaster);
		}
	    if(spell)
            with->AddSpell(spell,mCaster);
	...
	if(mCastingItem->Explosion)//eData.isEmpty())
	{
        ...
        getPositionOrientation(pos,ornt);
		//mDebugText(ogre_str(pos));
        new ExplosionObject(mCaster,with,mCastingItem,mLevel,pos,true,false,ornt);
	}
