...
void BeamObject::init(GameObject * _caster, gamedata_damagedealer *_source, Level *lvl,int SaveGameID)
{
...
//
	...
	if(source->Explosion)
	{
        mEffect = new ExplosionObject(caster,NULL,source,mLevel,pos,true,true);
	}
...
void BeamObject::update(Ogre::Real time)
{
    ...
    static_cast<GameChar*>(caster)->getTargetData(ornt,target,dist);
    //ornt = ornt.Inverse();
    ornt = static_cast<GameChar*>(caster)->getFiringOrientation();
